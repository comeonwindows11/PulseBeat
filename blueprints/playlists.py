...
def playlist_hidden_by_feature_toggle(playlist):
    return (not is_youtube_integration_enabled(True)) and is_youtube_linked_playlist(playlist)
...
def list_playlists():
    ...
    query = compose_and_filters(base_query, youtube_playlist_visibility_clause()) or base_query
...
def suggest_playlists():
    ...
    query = compose_and_filters({"$or": [{"user_id": user_oid}, {"collaborator_ids": user_oid}]}, youtube_playlist_visibility_clause()) or {
        "$or": [{"user_id": user_oid}, {"collaborator_ids": user_oid}]
    }
    if q:
        safe_q = re.escape(q[:80])
        query = compose_and_filters(query, {"name": {"$regex": safe_q, "$options": "i"}}) or query
...
@login_required
def quick_create_playlist_with_song():
    user_oid = get_session_user_oid()
    payload = request.get_json(silent=True) or {}

    name = (payload.get("name", "") or "").strip()
    if not name:
        return jsonify({"ok": False, "message": tr("flash.playlists.name_required")}), 400
    ...
    return jsonify(
        {
            ...
            "message": tr("flash.playlists.created_with_song") if song_ids else tr("flash.playlists.created"),
        }
    )
...
def delete_playlist(playlist_id):
    ...
    playlist = extensions.playlists_col.find_one({"_id": playlist_oid})
    if not playlist or playlist_hidden_by_feature_toggle(playlist) or not is_playlist_owner(playlist, user_oid):
...
def playlist_search_suggest(playlist_id):
    ...
    if not playlist or playlist_hidden_by_feature_toggle(playlist) or not can_access_playlist(playlist, user_oid):
        return jsonify({"items": []})
